<template>
	<client-only>
		<Dialog :open="modal.open && modal.modal === `CustomGradient`" @close="useUpdateModal(false)"
			...
			<div class="fixed inset-0 flex items-center justify-center p-4">
				<DialogPanel class="w-full max-w-md drop-shadow-2xl drop-shadow-mantle">
					<div class="rounded-t-lg p-4 bg-base">
						<DialogTitle class="text-2xl font-semibold">Add color to gradient</DialogTitle>
						<DialogDescription class="text-lg text-subtext0">
							Use the color picker below to add a color to the gradient.
						</DialogDescription>

						<div class="mt-2 flex justify-center">
							<ColorPicker :color="color" alphaChannel="hide" defaultFormat="hex"
								:visibleFormats="['hex']" @color-change="updateColor">
